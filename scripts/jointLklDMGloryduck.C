...
void jointLklDMGloryduck(TString configFileName="$GLIKESYS/rcfiles/jointLklDM.rc",Int_t seed=-1)
{
  setDefaultStyle();
  ...
  Double_t logJ              = env->GetValue("jointLklDM.logJ",19.);
  ...
  for(Int_t imass=0;imass<nmass;imass++)
	...
	for(Int_t isample=0;isample<nsamples;isample++)
      ...
      // set and create all necessary values
      Int_t nPoints = 200;
      Double_t sigmaVlow = 1.e-28;
      Double_t sigmaVupp = 2.e-22;

      Double_t sigmaVstep  = (sigmaVupp-sigmaVlow)/(nPoints-1.);
      Int_t    point0 = TMath::FloorNint(-sigmaVlow/sigmaVstep);

      // create the array to hold the parabola sigmav values
      Double_t* sigmaVval = new Double_t[nPoints];
      ...
      if(imass==0)
        {
          ...
          for(Int_t ipoint=0;ipoint<nPoints;ipoint++)
            {
              sigmaVval[ipoint] = (ipoint==point0? 0 :sigmaVlow+sigmaVstep*ipoint);
              myfile << sigmaVval[ipoint] << " ";
            }
